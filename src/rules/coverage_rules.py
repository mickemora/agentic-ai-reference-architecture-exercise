...
def evaluate_coverage(claim: WarrantyClaim, vehicle: VehicleDetails) -> CoverageDecision:
    ...
    if (
        ...
        or abs(claim.reported_mileage - vehicle.mileage) > rules["mileage_conflict_tolerance"]
    ):
    ...
    if reasons:
        return CoverageDecision(
            ...
            explanation="The claim requires human review because evidence is incomplete, ambiguous, or conflicting.",
            missing_evidence=missing,
            requires_human_review=True,
            policy_references=[rules["policy_reference"]],
        )

    age_months = (
        (claim.repair_date.year - vehicle.in_service_date.year) * 12
        + claim.repair_date.month
        - vehicle.in_service_date.month
    )
    ...
    if denial_reasons:
        return CoverageDecision(
            ...
            explanation="The claim does not satisfy the synthetic coverage rules.",
            missing_evidence=[],
    ...
    return CoverageDecision(
        ...
        explanation="The claim satisfies the synthetic coverage rules.",
        missing_evidence=[],
        requires_human_review=False,
        policy_references=[rules["policy_reference"]],
    )
